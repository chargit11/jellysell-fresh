...
export default async function handler(req, res) {
  ...
  try {
    const { listing_id, quantity, user_id } = req.body;

    console.log('Received request:', { listing_id, quantity, user_id });
...
<ReviseItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  <Item>
    ...
    return res.status(200).json({ success: true, message: 'Quantity updated on eBay' });
  } catch (error) {
